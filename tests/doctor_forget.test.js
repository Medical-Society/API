...
describe('POST /api/v1/doctors/login', () => {
    test('should return 200 success on not existing email', async () => {
        ...
        expect(res.body).toHaveProperty('message', 'Reset password link sent to vv@gmail.com');
    });
    ...
    test('should return 200 success on existing email', async () => {
        ...
        expect(res.status).toEqual(200);
        expect(res.body).toHaveProperty('message', 'Reset password link sent to fikif86177@tospage.com');
    });
});
